...
int search_symtab(char operand[20], char *operandadd){
    ...
    while(!feof(symtab)){
        ...
        fscanf(symtab, "%s %s", operandadd, symbolname);
...
int main(){
    ...
    char programname[20], opcode[20], operand[20], label[20], optabvalue[20], operandadd[20], objcode[20], textrec[60];
    int startingadd, flen, loc, txtcnt=60;
    strcpy(textrec, "");


    ifile = fopen("twopassinput.txt", "r");
    listingfile = fopen("listingfile.txt", "w");
    objectcodefile = fopen("output.txt", "w");
    length = fopen("length.txt", "r");

    fscanf(length, "%d", &flen);
    fscanf(ifile, "%x %s %s %x", &loc, programname, opcode, &startingadd);
